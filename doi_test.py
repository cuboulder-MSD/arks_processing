    ...
    try:
        with open(input_csv, mode='r', encoding='utf-8-sig') as f_in, \
            ...
            for i, row in enumerate(reader, 1):
                ...
                raw_type = (row.get('Resource Type#1') or row.get('Resource Type') or row.get('Type#1') or row.get('Type') or row.get('Type of Resource#1') or row.get('Type of Resource') or row.get('Work Type#1') or row.get('Work Type') or row.get('formatMediaType#1') or row.get('formatMediaType#2') or row.get('formatMediaType') or row.get('IANA Media Type#1') or '').strip()
